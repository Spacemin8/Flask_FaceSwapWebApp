...
class FaceSwap:
    ...
    def get_landmarks(self, im):
        ...
        return np.matrix([[p.x, p.y] for p in self.predictor(im, rects[0]).parts()]), rects[0]
    ...
    def __init__(self, image1, image2):
        ...
        im1, landmarks1,rect1 = self.read_im_and_landmarks(str(image1))
        name=self.save_landmark_image(im1, landmarks1, rect1, image1)
        im2, landmarks2,rect2 = self.read_im_and_landmarks(str(image2))
    ...
    def save_landmark_image(self, img, landmarks, rect, original_image_path):
        img_with_landmarks = img.copy()
        i1, _ = self.generate_name(original_image_path, original_image_path)
        landmarksFilename=f'{i1}_with_landmarks.jpg'
        self.draw_rectangle(img_with_landmarks, rect)
        landmarks_list = [(int(point[0, 0]), int(point[0, 1])) for point in landmarks]
        self.draw_landmarks(img_with_landmarks, landmarks_list)
        # Save images with landmarks and rectangles
        cv2.imwrite(os.path.join(os.getcwd(), 'static', 'output', landmarksFilename), img_with_landmarks)
        print(f"Landmarks and rectangle added to image: {landmarksFilename}")
        return landmarksFilename
    
    def landmarkDisplay(self,image1):
        im1, landmarks1,rect1 = self.read_im_and_landmarks(str(image1))
        name=self.save_landmark_image(im1, landmarks1, rect1, image1)
        return name
